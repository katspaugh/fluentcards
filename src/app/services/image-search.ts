...
export class ImageSearchService {
    ...
    getImages(word: string) {
        return this.makeRequest({ q: word, count: 10 })
            .map((data) => {
                if (!data.value.length) return Observable.throw(new Error(notFoundError));
    ...
    getSingleImage(word: string) {
        return this.makeRequest({ q: word, count: 1 })
            .map((data) => {
                let item = data.value[0];

                if (!item) return Observable.throw(new Error(notFoundError));
